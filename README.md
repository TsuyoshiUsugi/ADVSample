...
会話データシートに記載された **背景画像名** や **キャラクター画像名＋感情キー** は、すべて **アドレッサブル経由でロード** されます。  
正しく表示させるためには、以下の手順に従って設定してください。

#### 1. アドレッサブルへの登録

- Unityエディタで使用する画像（`Sprite` や `Texture2D` など）を選択します。
- **Addressable** にチェックを入れ、**Addressable Groups** ウィンドウでグループに追加します。

#### 2. **共通ラベルの設定（重要）**

- **背景画像**・**キャラクター画像**は **一括でロードされるため、すべて同じラベル** を設定してください。  
  例：`SkitImageAssets`
- このラベルは、`SkitSceneStarter` コンポーネントの **インスペクタ上の「_skitSceneImageAddressablePath」欄** に設定できます。
- `SkitSceneStarter` は、シーン開始時にこのラベルが付けられた画像アセットを一括ロードします。

> ⚠ **注意**：ラベルが設定されていない場合、画像が正しく読み込まれず表示されません。

#### 3. シートへの記述

- **背景画像**は、会話データシートの `background` 列に画像名(拡張子なし)を記述します。
- **キャラクター画像**は、全般設定データシートで「キャラ名＋感情キー」に対応する画像名(拡張子なし）を記述します。
